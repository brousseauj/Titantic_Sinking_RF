df.train = read.csv('~/Desktop/Portfolio/Titantic/train.csv')
df.test = read.csv('~/Desktop/Portfolio/Titantic/test (1).csv')
...
all.df$title = as.factor(regmatches(
  all.df$Name,
  regexpr("(?<=, ).+?(?=\\.)", all.df$Name, perl = TRUE)
))
...
age_plot =
  ...
  xlab('') + ylab("Age") + ggtitle('Age of Passenger') + theme(axis.title =
...
fare_plot =
  ...
  xlab('') + ylab("Fare") + ggtitle('Fare of Passenger') + theme(axis.title =
                                                                   element_text(size = 12))
...
class_count =
  ...
  xlab('') + facet_grid( ~ Survived) +
  theme(axis.title = element_text(size = 12))
ggsave('class_count.png', plot = class_count, dpi = 400)


embarked_plot =
  ggplot(train.df, aes(Embarked)) + geom_bar() + facet_grid( ~ Survived) +
  theme_fivethirtyeight() +
  ggtitle('Port of Embarkation\n(C = Cherbourg; Q = Queenstown; S = Southampton)') +
  theme(axis.title = element_text(size = 12))
ggsave('plcass_plot.png', plot = embarked_plot, dpi = 300)

title_plot =
  ggplot(train.df, aes(title)) + geom_bar() + facet_grid( ~ Survived) +
  theme_fivethirtyeight() +
...
familysize_plot =
  ...
  facet_grid( ~ Survived) + theme_fivethirtyeight() +
  xlab('') + ylab("Size") + ggtitle('Family Size of Passenger') + theme(axis.title =
                                                                          element_text(size = 12))
...
socioEconFeat_plot =
  ...
  xlab('') + ylab("Socioeconomic Rating") + theme(axis.title = element_text(size =
                                                                              12))
ggsave('sociofeat.png', socioEconFeat_plot, dpi = 300)

# Plot difference in tree age vs median fill age

tempdf = as.data.frame(rbind(
  cbind(all.df$Age, 'Filled_with_Median'),
  cbind(all.df$treeAge, 'Filled_with_Decision_Tree')
))
...
fit1a = randomForest(
  Survived ~
    Sex + title + Fare + SocioFeat + treeAge + Pclass +
    familySize +
